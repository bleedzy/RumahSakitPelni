...
class formPermintaanTindakanKoreksiController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = formPermintaanTindakanKoreksi::with('user');
            return DataTables::of($query)
                ->addColumn('action', function ($row) {
                    return '
                        <div class="flex gap-1 ml-1">
                            <a href="' . route('02.01.show', encrypt($row->id)) . '" class="text-blue-700">
                ...
                ->editColumn('tanggal_ptk', function ($row) {
                    return Carbon::parse($row->tanggal_ptk)->format('d-M-Y');
                })
