...
class formJadwalTahunanAuditInternalController extends Controller
{
    /**
    ...
    {
        // $data = formJadwalTahunanAuditInternal::all();
        // dd($data);
        if ($request->ajax()) {
            $query = formJadwalTahunanAuditInternal::with('user');
            return DataTables::of($query)
                ->addColumn('action', function ($row) {
                    return '
                        <div class="flex gap-1 ml-1">
                            <a href="' . route('03.03.show', encrypt($row->id)) . '" class="text-blue-700">
