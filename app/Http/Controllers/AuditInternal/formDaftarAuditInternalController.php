...
class formDaftarAuditInternalController extends Controller
{
    /**
    ...
    {
        // $data = formDaftarAuditInternal::all();
        // dd($data);
        if ($request->ajax()) {
            $query = formDaftarAuditInternal::with('user');
            return DataTables::of($query)
                ->addColumn('action', function ($row) {
                    return '
                        <div class="flex gap-1 ml-1">
                            <a href="' . route('03.01.show', encrypt($row->id)) . '" class="text-blue-700">
