...
class formPtkAuditInternalController extends Controller
{
    /**
    ...
    {
        // $data = formPtkAuditInternal::all();
        // dd($data);
        if ($request->ajax()) {
            $query = formPtkAuditInternal::with('user');
            return DataTables::of($query)
                ->addColumn('action', function ($row) {
                    return '
                        <div class="flex gap-1 ml-1">
                            <a href="' . route('03.02.show', encrypt($row->id)) . '" class="text-blue-700">
                ...
                ->editColumn('tanggal_audit', function ($row) {
                    return empty($row->tanggal_audit) ? '-' : Carbon::parse($row->tanggal_audit)->format('d-M-Y');
                })
                ->editColumn('tanggal_batas_penyelesaian', function ($row) {
                    return empty($row->tanggal_batas_penyelesaian) ? '-' : Carbon::parse($row->tanggal_batas_penyelesaian)->format('d-M-Y');
                })
                ->editColumn('tanggal_penyelesaian', function ($row) {
                    return empty($row->tanggal_penyelesaian) ? '-' : Carbon::parse($row->tanggal_penyelesaian)->format('d-M-Y');
                })
                ->editColumn('tanggal_verifikasi', function ($row) {
                    return empty($row->tanggal_verifikasi) ? '-' : Carbon::parse($row->tanggal_verifikasi)->format('d-M-Y');
                })
